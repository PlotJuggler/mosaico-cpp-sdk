...
bool parseEndpointInfo(const std::string& metadata, int64_t& ts_min, int64_t& ts_max, int64_t& total_bytes) {
  if (metadata.empty()) {
    return false;
  }
...
MosaicoClient::MosaicoClient(
    const std::string& server_uri, int timeout_seconds, size_t pool_size, const std::string& tls_cert_path,
...
arrow::Status MosaicoClient::doAction(
    fl::FlightClient* client, const std::string& action_type, const std::string& json_body, std::string* response) {
  fl::Action action;
...
arrow::Result<std::vector<SequenceInfo>> MosaicoClient::listSequences(
    SequenceListStartedCallback on_list_started, SequenceInfoCallback on_sequence_info) {
  ...
  parallelIndexed(names.size(), pool_.poolSize(), [&](size_t i) {
    ...
    auto descriptor = fl::FlightDescriptor::Command(json{{"resource_locator", names[i]}}.dump());
    ...
    if (!info_result.ok()) {
      ...
      MOSAICO_SDK_LOG(
          ...
          tidStr().c_str(), i, names[i].c_str(), (long long)ms_info, info_result.status().ToString().c_str());
      emit_sequence_info(sequences[i]);
      return;
    }
    ...
    MOSAICO_SDK_LOG(
        ...
        tidStr().c_str(), i, names[i].c_str(), (long long)ms_info, (long long)sdkElapsedMs(t_pick));
  ...
  MOSAICO_SDK_LOG(
      ...
      (long long)ms_getinfo_all, (long long)getinfo_ok.load(), (long long)getinfo_fail.load(),
      names.empty() ? 0.0 : static_cast<double>(ms_getinfo_all) / names.size(), pool_.poolSize(), (long long)ms_total);
...
arrow::Result<std::vector<TopicInfo>> MosaicoClient::listTopics(const std::string& sequence_name) {
  ...
  ARROW_ASSIGN_OR_RAISE(auto conn, pool_.checkout());
  auto descriptor = fl::FlightDescriptor::Command(json{{"resource_locator", sequence_name}}.dump());
  ...
  MOSAICO_SDK_LOG(
      ...
      sequence_name.c_str(), (long long)ms_info, (long long)sdkElapsedMs(t_begin), (long long)endpoint_count,
      (long long)topics.size());
  return topics;
}
...
arrow::Result<TopicInfo> MosaicoClient::getTopicMetadata(
  ...
  ARROW_ASSIGN_OR_RAISE(auto conn, pool_.checkout());
  auto descriptor =
      fl::FlightDescriptor::Command(json{{"resource_locator", packResource(sequence_name, topic_name)}}.dump());
  ...
  MOSAICO_SDK_LOG(
      ...
      "OK in %lld ms\n",
      sequence_name.c_str(), topic_name.c_str(), (long long)ms_info, (long long)sdkElapsedMs(t_begin));
  return topic;
}
...
arrow::Result<PullResult> MosaicoClient::pullTopic(
    ...
    bool retain_batches) {
  auto is_cancelled = [interrupted]() { return interrupted && interrupted->load(std::memory_order_relaxed); };
  const auto t_begin = sdkNow();
  ...
  MOSAICO_SDK_LOG(
      ...
      sequence_name.c_str(), topic_name.c_str(), tid.c_str(), (long long)ms_checkout, (long long)ms_info);
  ...
  while (true) {
    if (is_cancelled()) {
      ...
      MOSAICO_SDK_LOG(
          ...
          sequence_name.c_str(), topic_name.c_str(), tid.c_str(), (long long)sdkElapsedMs(t_begin),
          (long long)chunk_count, (long long)total_bytes);
    ...
    ++chunk_count;
    if (ms_first_chunk < 0) {
      ms_first_chunk = std::chrono::duration_cast<std::chrono::milliseconds>(t_got - t_stream_begin).count();
    } else {
      const auto gap = std::chrono::duration_cast<std::chrono::milliseconds>(t_got - last_chunk_time).count();
      if (gap > max_gap_ms) {
        max_gap_ms = gap;
      }
    }
  ...
  MOSAICO_SDK_LOG(
      ...
      sequence_name.c_str(), topic_name.c_str(), tid.c_str(), (long long)ms_doget, (long long)ms_schema,
      (long long)ms_stream, (long long)chunk_count, (long long)total_rows, mb, mb_per_sec, (long long)ms_first_chunk,
...
arrow::Status MosaicoClient::pullTopics(
    const std::string& sequence_name, const std::vector<std::string>& topic_names, const TimeRange& range,
    TopicCompleteCallback on_topic_done, MultiTopicProgressCallback progress_cb, std::atomic<bool>* interrupted,
    MultiTopicBatchCallback batch_cb, MultiTopicSchemaCallback schema_cb, bool retain_batches) {
  const auto t_begin = sdkNow();
  const size_t parallelism = std::min<size_t>(topic_names.size(), pool_.poolSize());
  {
    ...
    MOSAICO_SDK_LOG(
        ...
        "topics=[%s]\n",
        sequence_name.c_str(), (long long)topic_names.size(), parallelism, pool_.poolSize(), tidStr().c_str(),
        topics_csv.c_str());
  }
  ...
  parallelIndexed(topic_names.size(), pool_.poolSize(), [&](size_t i) {
    ...
    MOSAICO_SDK_LOG(
        ...
        sequence_name.c_str(), tidStr().c_str(), i, topic.c_str(), my_pick, (long long)topic_names.size(),
    ...
    auto result = pullTopic(
        sequence_name, topic, range, std::move(per_topic_progress), interrupted, std::move(per_topic_batch),
        std::move(per_topic_schema), retain_batches);

    MOSAICO_SDK_LOG(
        "[Mosaico SDK] pullTopics %s: tid=%s released topic[%zu]=%s "
        "after %lld ms (status=%s)\n",
        sequence_name.c_str(), tidStr().c_str(), i, topic.c_str(), (long long)sdkElapsedMs(t_pick),
  ...
  MOSAICO_SDK_LOG(
      ...
      (long long)sdkElapsedMs(t_begin), (long long)topic_names.size());
...
arrow::Status MosaicoClient::reportSequenceNotification(
  ...
  ARROW_ASSIGN_OR_RAISE(auto conn, pool_.checkout());
  json payload = {{"locator", sequence_name}, {"notification_type", type}, {"msg", message}};
  return doAction(&*conn, "sequence_notification_create", payload.dump());
}

arrow::Status MosaicoClient::reportTopicNotification(
    const std::string& sequence_name, const std::string& topic_name, const std::string& type,
    const std::string& message) {
  ARROW_ASSIGN_OR_RAISE(auto conn, pool_.checkout());
  json payload = {{"locator", packResource(sequence_name, topic_name)}, {"notification_type", type}, {"msg", message}};
  return doAction(&*conn, "topic_notification_create", payload.dump());
}

arrow::Result<std::vector<Notification>> MosaicoClient::listSequenceNotifications(const std::string& sequence_name) {
  ARROW_ASSIGN_OR_RAISE(auto conn, pool_.checkout());
...
arrow::Status MosaicoClient::purgeTopicNotifications(const std::string& sequence_name, const std::string& topic_name) {
  ARROW_ASSIGN_OR_RAISE(auto conn, pool_.checkout());
  json payload = {{"locator", packResource(sequence_name, topic_name)}};
...
arrow::Result<ApiKeyInfo> MosaicoClient::createApiKey(
    ApiKeyPermission permission, const std::string& description, std::optional<int64_t> expires_at_ns) {
  ARROW_ASSIGN_OR_RAISE(auto conn, pool_.checkout());

  json request_payload = {{"permissions", toString(permission)}, {"description", description}};
