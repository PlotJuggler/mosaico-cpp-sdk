...
std::optional<std::string> extractOntologyTag(const std::shared_ptr<arrow::KeyValueMetadata>& metadata) {
  if (!metadata) {
    return std::nullopt;
  }
...
std::optional<std::string> detectOntologyTag(const std::shared_ptr<arrow::Schema>& schema) {
  if (!schema) {
    return std::nullopt;
  }

  auto has_field = [&](const std::string& name) -> bool { return schema->GetFieldIndex(name) >= 0; };
  ...
  // Image / compressed image: single binary "data" field
  {
    ...
    if (data_idx >= 0 && schema->num_fields() <= 3) {
      ...
      if (field->type()->id() == arrow::Type::BINARY || field->type()->id() == arrow::Type::LARGE_BINARY) {
