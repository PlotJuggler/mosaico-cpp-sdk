...
inline std::optional<std::string> tryGetString(const json& parent, const std::string& key) {
  if (!parent.is_object()) {
    return std::nullopt;
  }
  if (!parent.contains(key)) {
...
inline std::optional<int64_t> tryGetInt64(const json& parent, const std::string& key) {
  if (!parent.is_object()) {
    return std::nullopt;
  }
  if (!parent.contains(key)) {
...
inline std::optional<uint64_t> tryGetUint64(const json& parent, const std::string& key) {
  if (!parent.is_object()) {
    return std::nullopt;
  }
  if (!parent.contains(key)) {
...
inline std::optional<bool> tryGetBool(const json& parent, const std::string& key) {
  if (!parent.is_object()) {
    return std::nullopt;
  }
  if (!parent.contains(key)) {
