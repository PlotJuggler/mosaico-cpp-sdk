...
class ApiKeyMiddleware : public fl::ClientMiddleware {
 public:
  explicit ApiKeyMiddleware(std::string api_key) : api_key_(std::move(api_key)) {}
...
class ApiKeyMiddlewareFactory : public fl::ClientMiddlewareFactory {
 public:
  explicit ApiKeyMiddlewareFactory(std::string api_key) : api_key_(std::move(api_key)) {}

  void StartCall(const fl::CallInfo&, std::unique_ptr<fl::ClientMiddleware>* middleware) override {
...
std::string sanitizeHeaderValue(const std::string& s) {
  const auto is_trim = [](unsigned char c) { return c == ' ' || c == '\t' || c == '\r' || c == '\n'; };
  size_t begin = 0;
...
arrow::Result<std::unique_ptr<fl::FlightClient>> ConnectionPool::createConnection() {
  ...
  if (!tls_cert_path_.empty()) {
    ...
    if (!f) {
      return arrow::Status::IOError("Cannot open TLS certificate file: " + tls_cert_path_);
    }
    std::stringstream ss;
    ss << f.rdbuf();
  ...
  if (!api_key_.empty()) {
    opts.middleware.push_back(std::make_shared<ApiKeyMiddlewareFactory>(api_key_));
  }
...
arrow::Result<ConnectionPool::Handle> ConnectionPool::checkout() {
  ...
  const std::string tid = poolTidStr();
  {
    ...
    for (size_t i = 0; i < pool_size_; ++i) {
      if (slot_state_[i] == SlotState::Free && clients_[i]) {
        ...
        const auto wait_ms =
            std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - t_begin).count();
        MOSAICO_SDK_LOG(
    ...
    for (size_t i = 0; i < pool_size_; ++i) {
      if (slot_state_[i] == SlotState::Free && !clients_[i]) {
        ...
        const auto connect_ms =
            std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - t_connect).count();
        lock.lock();
        ...
        if (!result.ok()) {
          ...
          MOSAICO_SDK_LOG(
              ...
              tid.c_str(), i, (long long)connect_ms, result.status().ToString().c_str());
        ...
        MOSAICO_SDK_LOG(
            ...
            tid.c_str(), i, (long long)connect_ms, (long long)wait_ms);
        return Handle(this, i, clients_[i].get());
      }
    }
  }
  ...
  const auto overflow_ms =
      std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - t_overflow).count();
  if (!overflow_result.ok()) {
    MOSAICO_SDK_LOG(
        "[Mosaico SDK] pool: tid=%s OVERFLOW connect FAILED "
        "(%lld ms): %s\n",
        tid.c_str(), (long long)overflow_ms, overflow_result.status().ToString().c_str());
...
ConnectionPool::Handle::Handle(ConnectionPool* pool, size_t index, fl::FlightClient* client)
    : pool_(pool), index_(index), client_(client), overflow_(false) {}

ConnectionPool::Handle::Handle(std::unique_ptr<fl::FlightClient> overflow_client)
...
bool ConnectionPool::Handle::valid() const {
