...
class MosaicoClient {
 public:
  ...
  MosaicoClient(
      const std::string& server_uri, int timeout_seconds, size_t pool_size = 4, const std::string& tls_cert_path = "",
  ...
  // Discovery
  using SequenceListStartedCallback = std::function<void(const std::vector<SequenceInfo>& sequences)>;
  using SequenceInfoCallback = std::function<void(const SequenceInfo& sequence, int64_t completed, int64_t total)>;

  arrow::Result<std::vector<SequenceInfo>> listSequences(
      SequenceListStartedCallback on_list_started = nullptr, SequenceInfoCallback on_sequence_info = nullptr);
  ...
  arrow::Result<TopicInfo> getTopicMetadata(const std::string& sequence_name, const std::string& topic_name);
  ...
  arrow::Result<PullResult> pullTopic(
      ...
      ProgressCallback progress_cb = nullptr, std::atomic<bool>* interrupted = nullptr,
      BatchCallback batch_cb = nullptr, SchemaCallback schema_cb = nullptr, bool retain_batches = true);
  ...
  using TopicCompleteCallback = std::function<void(const std::string& topic_name, arrow::Result<PullResult> result)>;
  using MultiTopicProgressCallback =
      std::function<void(const std::string& topic_name, int64_t rows, int64_t bytes, int64_t total_bytes)>;
  using MultiTopicBatchCallback =
      std::function<void(const std::string& topic_name, const std::shared_ptr<arrow::RecordBatch>& batch)>;
  using MultiTopicSchemaCallback =
      std::function<void(const std::string& topic_name, const std::shared_ptr<arrow::Schema>& schema)>;
  ...
  arrow::Status pullTopics(
      ...
      TopicCompleteCallback on_topic_done = nullptr, MultiTopicProgressCallback progress_cb = nullptr,
  ...
  arrow::Status reportTopicNotification(
      const std::string& sequence_name, const std::string& topic_name, const std::string& type,
      const std::string& message);
  arrow::Result<std::vector<Notification>> listSequenceNotifications(const std::string& sequence_name);
  ...
  arrow::Status purgeTopicNotifications(const std::string& sequence_name, const std::string& topic_name);

  // API key management (requires Manage tier).
  arrow::Result<ApiKeyInfo> createApiKey(
      ApiKeyPermission permission, const std::string& description, std::optional<int64_t> expires_at_ns = std::nullopt);
